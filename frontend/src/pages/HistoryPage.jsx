...
export default function HistoryPage() {
  ...
  const formatDate = dateStr => { try { return new Date(dateStr).toLocaleString('id-ID') } catch { return dateStr } }
            ...
            <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #e2e8f0', overflowX: 'auto' }}>
              {files.length > 0 ? (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                  ...
                  <tbody>
                    {files.map(file => (
                      <tr key={file.name} onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'} onMouseLeave={e => e.currentTarget.style.background = ''}>
                        ...
                        <td style={{ padding: '12px 14px', borderBottom: '1px solid #f1f5f9' }}>
                          <a href={file.path} download style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#eff6ff', color: '#2563eb', border: 'none', padding: '6px 14px', borderRadius: '8px', fontWeight: 700, fontSize: '12px', textDecoration: 'none' }}>
                ...
                <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#94a3b8' }}>
                  ...
                  <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#2563eb', color: 'white', padding: '8px 18px', borderRadius: '10px', fontWeight: 700, fontSize: '13px', textDecoration: 'none' }}>
