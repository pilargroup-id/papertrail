...
import DataTablePaymentMethod from '../../../../components/table/master-table/payment-methods/DataTablePaymentMethod.jsx'

import DialogEditPaymentMethods from '../../../../components/Dialog/dialog-payment-methods/DialogEditPaymentMethods.jsx'

import ButtonCreatePaymentMethods from '../../../../components/button/button-payment-methods/ButtonCreatePaymentMethods.jsx'
...
function getRuleLabel(rule) {
  return [
    rule?.code,
    rule?.name,
  ].filter(Boolean).join(' - ') || 'payment method ini'
}
...
function FrpDocumentsTypePage(props) {
  ...
  useEffect(() => {
    ...
    async function loadFrpDocumentsTypeRules() {
      ...
      try {
        ...
        setErrorMessage(error.message || 'Gagal memuat data payment methods.')
      } finally {
  ...
  const handleRuleStatusChange = async (budgetAccessRule, nextIsActive) => {
    ...
    try {
      const response = await api.paymentMethods.updateStatus(budgetAccessRuleId, normalizedIsActive)
      const updatedRule = getRuleFromResponse(response)
      ...
      setErrorMessage(error.message || 'Gagal memperbarui status payment method.')
    } finally {
