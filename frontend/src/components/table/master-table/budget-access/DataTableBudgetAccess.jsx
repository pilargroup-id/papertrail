...
function getBanksStatusLabel(budgetAccessRules) {
  return Number(budgetAccessRules?.is_active) === 1 ? 'Aktif' : 'Nonaktif'
}

function getBanksStatusVariant(budgetAccessRules) {
  return Number(budgetAccessRules?.is_active) === 1 ? 'active' : 'inactive'
}

function getIsBanksActive(budgetAccessRules) {
  return Number(budgetAccessRules?.is_active) === 1
}

function getVendorBankPrimaryLabel(budgetAccessRules) {
  return Number(budgetAccessRules?.is_primary) === 1 ? 'Primary' : 'Secondary'
}

function getVendorBankPrimaryVariant(budgetAccessRules) {
  return Number(budgetAccessRules?.is_primary) === 1 ? 'active' : 'inactive'
}
...
function renderBanksStatus(budgetAccessRules, index, {
  isStatusUpdating,
  onStatusChange,
  SwitchComponent,
}) {
  const isActive = getIsBanksActive(budgetAccessRules)
  const isUpdating =
  ...
  const statusLabel = getBanksStatusLabel(budgetAccessRules)
  const statusPill = (
    <DataTableStatus
      className="banks-status-toggle__pill"
      variant={getBanksStatusVariant(budgetAccessRules)}
    >
      {statusLabel}
    </DataTableStatus>
  )
    ...
    <>
      {SwitchComponent ? (
        <SwitchComponent
          ...
          checked={isActive}
          disabled={isDisabled}
          aria-label={`Ubah status banks ${budgetAccessRules?.name ?? budgetAccessRules?.id ?? index}`}
        ...
        column.key === 'status'
          ? {
              ...
              render: (budgetAccessRules, index) =>
                renderBanksStatus(budgetAccessRules, index, {
                  isStatusUpdating,
                  onStatusChange,
                  SwitchComponent,
                }),
    ...
    shouldRenderStatusSwitch && mobileCard !== false
      ? {
          ...
          header: {
            ...(mobileCard?.header ?? {}),
            status: {
              label: (budgetAccessRules) => getBanksStatusLabel(budgetAccessRules),
              variant: (budgetAccessRules) => getBanksStatusVariant(budgetAccessRules),
