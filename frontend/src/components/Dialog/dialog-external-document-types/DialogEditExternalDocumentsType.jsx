...
function DialogEditExternalDocumentsType({
  isOpen = false,
  eyebrow = 'FRP Document Types',
  title = 'Edit FRP Document Type',
  user = null,
  vendor = null,
  budgetType = null,
  frpDocumentType = null,
  onClose,
  onUpdated,
}) {
  ...
  const resolvedFrpDocumentType = frpDocumentType ?? budgetType ?? vendor ?? user
  ...
  useEffect(() => {
    ...
    setFormValues({
      code: resolvedFrpDocumentType?.code ?? '',
      name: resolvedFrpDocumentType?.name ?? '',
      description: resolvedFrpDocumentType?.description ?? '',
      sort_order: resolvedFrpDocumentType?.sort_order ?? '',
    })
    setFieldErrors({})
    setSubmitError('')
  ...
  }, [isOpen, onClose, resolvedFrpDocumentType])

  const handleSubmit = async (event) => {
    event.preventDefault()

    const frpDocumentTypeId = resolvedFrpDocumentType?.id
    ...
    if (frpDocumentTypeId === undefined || frpDocumentTypeId === null) {
      setSubmitError('Data FRP document type tidak ditemukan.')
      return
    }
    ...
    if (!normalizedName) {
      nextFieldErrors.name = 'Name FRP document type wajib diisi.'
    }
    ...
    try {
      const response = await api.externalDocumentTypes.update(frpDocumentTypeId, {
        code: normalizedCode,
        name: normalizedName,
      ...
      setSubmitError(error.message || 'Gagal memperbarui FRP document type.')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const dialogNode = (
    <div className="dashboard-popup-overlay" role="presentation" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-edit-frp-document-type-title"
        ...
        <form onSubmit={handleSubmit}>
          <div className="dashboard-popup__header">
            <div>
              ...
              <h2 className="dashboard-popup__title" id="dialog-edit-frp-document-type-title">
                {title}
              </h2>
            </div>
          </div>
                  ...
                  <div className="register-user-popup__grid register-user-popup__grid--vendor-banks">
                    <div className="register-user-popup__field">
                      <TextField
                        ...
                        placeholder="Input code document type"
                        leftIcon={Code}
                        required
                    ...
                    <div className="register-user-popup__field">
                      <TextField
                        label="Name Document Type"
                        value={formValues.name}
                        placeholder="Input name document type"
                        leftIcon={FileText01}
                        required
                        disabled={isFormDisabled}
...
export default DialogEditExternalDocumentsType
