...
function TabsItems({
  ...
  const wasOptionsLoadingRef = useRef(isOptionsLoading)
  ...
  useEffect(() => {
    const wasOptionsLoading = wasOptionsLoadingRef.current
    wasOptionsLoadingRef.current = isOptionsLoading
    ...
    if (wasOptionsLoading && !isOptionsLoading) {
      // The FRP just finished hydrating from the server — keep its saved
      // exchange rate instead of overwriting it with today's latest rate.
      return undefined
    }
  ...
  }, [currencyCode, formValues.frp_date, isOptionsLoading])
