...
function DialogCreateRpDestionationsDepartments({
  isOpen = false,
  eyebrow = 'RP Destination Departments',
  title = 'Create RP Destination Department',
  onClose,
  onCreated,
}) {
  const [formValues, setFormValues] = useState(initialRpDestinationDepartmentFormValues)
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await api.rpDestinationDepartments.create({
      ...
      setSubmitError(error.message || 'Gagal membuat RP destination department.')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const dialogNode = (
    <div className="dashboard-popup-overlay" role="presentation" onClick={handleClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-create-rp-destination-department-title"
          ...
          <div className="dashboard-popup__header">
            <div>
              ...
              <h2
                ...
                id="dialog-create-rp-destination-department-title"
              >
                {title}
              </h2>
            </div>
...
export default DialogCreateRpDestionationsDepartments
