...
function DialogCreateBudgetAccesRules({
  ...
  const resolvedBudgetType = budgetType ?? vendor ?? user
  ...
  useEffect(() => {
    ...
    setFormValues({
      code: resolvedBudgetType?.code ?? '',
      name: resolvedBudgetType?.name ?? '',
      description: resolvedBudgetType?.description ?? '',
    })
    setFieldErrors({})
    setSubmitError('')
  ...
  }, [isOpen, onClose, resolvedBudgetType])

  const handleSubmit = async (event) => {
    event.preventDefault()

    const budgetTypeId = resolvedBudgetType?.id
    ...
    if (budgetTypeId === undefined || budgetTypeId === null) {
      setSubmitError('Data budget type tidak ditemukan.')
      return
    }
    ...
    if (normalizedDepartment.length > 255) {
    ...
    try {
      const response = await api.budgetTypes.update(budgetTypeId, {
        module: normalizedModule,
        accessType: normalizedAccessType,
        department: normalizedDepartment,
        departmentClass:normalizedDepartmentClass,
      })
      ...
      setSubmitError(error.message || 'Gagal memperbarui budget type.')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const dialogNode = (
    <div className="dashboard-popup-overlay" role="presentation" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-edit-budget-type-title"
        ...
        <form onSubmit={handleSubmit}>
          <div className="dashboard-popup__header">
            <div>
              ...
              <h2 className="dashboard-popup__title" id="dialog-edit-budget-type-title">
                {title}
              </h2>
            </div>
          </div>
